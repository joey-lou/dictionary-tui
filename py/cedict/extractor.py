"""Extract HeadEntry list from CC-CEDICT file (text or .gz)."""
...
def _parse_raw(path: Path, *, use_simplified: bool = True) -> list[_RawEntry]:
    if _is_gzip(path):
...
def parse_file(path: Path, *, use_simplified: bool = True) -> list[HeadEntry]:
    ...
    for p in phrases_raw:
        ...
        phrase_bucket[p.headword[0]].append(PhraseItem(form=p.headword, definition=f"[{p.pinyin_display}] {full}"))
...
class CEDICTExtractor(Extractor):
    """Extract HeadEntry list from a CEDICT file (text or .gz)."""

    def __init__(self, *, use_simplified: bool = True) -> None:
        self.use_simplified = use_simplified

    def extract(self, source: Path) -> list[HeadEntry]:
        return parse_file(source, use_simplified=self.use_simplified)
